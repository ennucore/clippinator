...
import { TermToolbox } from '../toolbox/editor';
...
export class ContextManagerTerm extends ContextManagerBase {
    ...
    async getHistoryPrefix(env: TermToolbox, commandDesc: string): Promise<Event[]> {
        ...
        if (this.promptManager.prompts['system']) {
            prefix.push({ type: "system", content: this.promptManager.getPrompt('system', {commands: commandDesc}) });
        }
        ...
        if (this.promptManager.prompts['demo']) {
            prefix.push({ type: "user", content: this.promptManager.getPrompt('demo', { }) });
        }
        prefix.push({ type: "user", content: this.promptManager.getPrompt('initial', { commands: commandDesc, workdir, workspace: workspaceSummary, objective: this.objective }) });
        return prefix;
    }
}
